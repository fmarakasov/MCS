...
namespace MContracts.ViewModel.Reports
{
    ...
    /// </summary>
    public class ContractQuarterPlanRepot_4_ViewModel : ExcelReportViewModel 
    {

        private int defaultstagerowwidth = 60;
        private int defaultcontractrowwidth = 320;

        public ContractQuarterPlanRepot_4_ViewModel(IContractRepository contractRepository): base(contractRepository)
        {
            PropertyChanged += new System.ComponentModel.PropertyChangedEventHandler(ContractQuarterPlanRepot_4_ViewModel_PropertyChanged);
        ...
        void ContractQuarterPlanRepot_4_ViewModel_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            if (e.PropertyName == "MainViewModel")
            {
                if (MainViewModel != null)
                {
                    ...
                    Quarter = DateTimeExtensions.GetQuarterByDate(MainViewModel.SelectedFilterEndDate);
                }
                else
                {
        ...
        private List<QuarterPlanContractDto> _contracts
        {
            get
            {
                var contractdocs = ActiveContractDocs.Where(cd => cd.Schedulecontracts != null && cd.Stages != null && cd.Stages.Count() > 0);
                           ...
                           contractdocs.Aggregate(new List<QuarterPlanContractDto>(), (a, c) =>
                                        {
                                            a.AddRange(c.Stages.Distinct().Where(st =>
                                                       ...
                                                       (st.GetConditionForPeriod(StartPeriod, EndPeriod) == StageCondition.Closed && st.GetConditionOnDate(StartPeriod) != StageCondition.Closed)).OrderBy(st => st.Startsat).Select(x =>
                                                       {
                                                           return
                                                               new QuarterPlanContractDto()
                                                               {
                                                                   ...
                                                                   Month1Price =  x.IsLeaf() ? x.PlanPriceOnPeriodWithNDS(DateTimeExtensions.GetFirstMonthDay(Year, (int)Quarter * 3 - 2), DateTimeExtensions.GetLastMonthDay(Year, (int)Quarter * 3 - 2), Measure) :0,

                                                                   Month2Price = x.IsLeaf() ? x.PlanPriceOnPeriodWithNDS(DateTimeExtensions.GetFirstMonthDay(Year, (int)Quarter * 3 - 1), DateTimeExtensions.GetLastMonthDay(Year, (int)Quarter * 3 - 1), Measure) : 0,

                                                                   Month3Price = x.IsLeaf() ? x.PlanPriceOnPeriodWithNDS(DateTimeExtensions.GetFirstMonthDay(Year, (int)Quarter * 3), DateTimeExtensions.GetLastMonthDay(Year, (int)Quarter * 3), Measure) : 0,
        ...
        protected override void SetHeader()
        {
            ...
            MainWorkSheet.Cells.Replace("#Quarter#", DateTimeExtensions.QuarterToRoman(Quarter));
            MainWorkSheet.Cells.Replace("#PreviousQuarter#", DateTimeExtensions.GetPreviousQuarters(Quarter));
        ...
        private void BuildContract(Contractdoc contract)
        {
            ...
            if (contract != null)
            {
                ...
                r.Replace("#Directors#", contract.Directors(true));
        ...
        private void BuildLeafStage(QuarterPlanContractDto stage)
        {
            ...
            if (stage.Stage.Act != null && stage.Stage.Act.Issigned.HasValue && stage.Stage.Act.Issigned.Value && stage.Stage.Act.Signdate.HasValue && stage.Stage.Act.Signdate.Value.Between(StartPeriod, EndPeriod))
            {
                tmpApprovalState =
                    Repository.ApprovalStates.FirstOrDefault(
                        p => ((WellKnownApprovalstates)p.Id == WellKnownApprovalstates.Signed));

                tmpApprovalDate = stage.Stage.Act.Signdate;
            }
            else
            {
                tmpApprovalState = stage.Stage.Approvalstate;
                tmpApprovalDate = stage.Stage.Statedate;
            }
            ...
            if (tmpApprovalState != null&&tmpApprovalDate.HasValue&&tmpApprovalDate.Value.Between(StartPeriod, EndPeriod))
            {
                ...
                if (iClosingAt > 0)
                {
                    var range = r.Item[1, iClosingAt] as Range;
                    if (range != null)
                    {
        ...
        private void BuildStatisticsHeader()
        {
            ...
            r.Replace("#Quarter#", DateTimeExtensions.QuarterToRoman(Quarter));
            r.Replace("#PreviousQuarter#", DateTimeExtensions.GetPreviousQuarters(Quarter));
