...
namespace MTests
{
    class ContractRepositorySubmitMock:IContractRepository
    {
        public int SubmitInvokedCount { get; private set; }
    ...
    [TestClass]
    public class AutoSubmitCommandTest
    {
        [TestMethod]
        public void TestSubmitAutoInvokedT()
        {
            int actionCount = 0;
            ContractRepositorySubmitMock mock = new ContractRepositorySubmitMock();
            AutoSubmitCommand<int> cmd = new AutoSubmitCommand<int>(x=>++actionCount, mock);
            cmd.Execute(actionCount);
            Assert.AreEqual(1, actionCount, "actionCount");
            Assert.AreEqual(1, mock.SubmitInvokedCount, "SubmitInvokedCount");
        }
        [TestMethod]
        public void TestSubmitAutoInvokedPolymorphyT()
        {
            int actionCount = 0;
            ContractRepositorySubmitMock mock = new ContractRepositorySubmitMock();
            ICommand cmd = new AutoSubmitCommand<int>(x => ++actionCount, mock);
            cmd.Execute(actionCount);
            Assert.AreEqual(1, actionCount, "actionCount");
            Assert.AreEqual(1, mock.SubmitInvokedCount, "SubmitInvokedCount");
        }
        [TestMethod]
        public void TestSubmitAutoInvoked()
        {
            int actionCount = 0;
            ContractRepositorySubmitMock mock = new ContractRepositorySubmitMock();
            AutoSubmitCommand cmd = new AutoSubmitCommand(x => ++actionCount, mock);
            cmd.Execute(actionCount);
            Assert.AreEqual(1, actionCount, "actionCount");
            Assert.AreEqual(1, mock.SubmitInvokedCount, "SubmitInvokedCount");
        }
        [TestMethod]
        public void TestSubmitAutoInvokedPolymorphy()
        {
            int actionCount = 0;
            ContractRepositorySubmitMock mock = new ContractRepositorySubmitMock();
            ICommand cmd = new AutoSubmitCommand(x => ++actionCount, mock);
            cmd.Execute(actionCount);
            Assert.AreEqual(1, actionCount, "actionCount");
            Assert.AreEqual(1, mock.SubmitInvokedCount, "SubmitInvokedCount");
        }
