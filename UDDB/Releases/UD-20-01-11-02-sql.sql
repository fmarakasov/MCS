...
Скрипт сгенерирован 07:39:05 PM 20/01/2011
Схема 1.1. 
Изменения:
 *Исправление ошибки в хранимой процедуре.
...
BEGIN
  ...
  if c > 0 then
    select SchemeRelease, SchemeBuild into Mj, Mn from UDMetadata where rownum=1  order by SchemeRelease, SchemeBuild desc;
    Mn:=Mn+1;
  else
    Mj:=1;
    Mn:=0;
