    ...
    public class ContractPeriodReport_3_ViewModel : ExcelReportViewModel
    {
        ...
        protected override string OutputFileName
        {
            get
            {
                ...
                return string.Format("Текущая справка на переходный период за {0} год", Year.ToString());
                    ...
                    var e =
                        contractdocs.Aggregate(new List<ContractDto>(), (a, c) =>
                        {
                            a.Add(new ContractDto()
                            {
                                ...
                                Price = c.GetPriceForPeriodWithNDS(StartPrevPeriod, EndPeriod, Measure) - c.GetPriceForPeriodWithNDS(StartPrevPeriod, EndPrevPeriod, Measure),

                                //Цена по  подписанным договорам
                                SignedContractsPrice = c.Contractstate == null ? 0 : !c.Contractstate.IsSigned ? 0 :
                                                       c.GetPriceForPeriodWithNDS(StartPrevPeriod, EndPeriod, Measure) - c.GetPriceForPeriodWithNDS(StartPrevPeriod, EndPrevPeriod, Measure),

                                // Цена по договорам с соисиполнителями
                                CoworkersPrice = c.Contractstate == null ? 0 : !c.Contractstate.IsSigned ? 0 :
                                                 c.GetCoworkersPriceForPeriodWithNDS(StartPrevPeriod, EndPeriod, Measure) - c.GetCoworkersPriceForPeriodWithNDS(StartPrevPeriod, EndPrevPeriod, Measure),
        ...
        protected override void BuildReport()
        {
            ...
            IEnumerable<IGrouping<Contractortype, ContractDto>> contractorTypeGroups =
                contractdocs.GroupBy(x => x.ContractorType, x => x).OrderBy(x => x.Key.Reportorder);
            foreach (IGrouping<Contractortype, ContractDto> contractorGroup in contractorTypeGroups)
            {
                ...
                IEnumerable<IGrouping<Contracttype, ContractDto>> contractTypeGroups =
